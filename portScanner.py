...
def scanner_port(cible, port, fichier_sortie, zone_resultats):
    try:
        ...
        if resultat == 0:
            ...
            message = f"[+] Port {port} ouvert - Service : {banniere}"
...
def lancer_scan():
    ...
    fichier_sortie = f"resultats_scan_{cible}.txt"
    with open(fichier_sortie, "w", encoding="utf-8") as f:
        f.write(f"Résultats du scan pour {cible} - {time.ctime()}\n\n")
...
def ouvrir_interface_principale(type_scan):
    fenetre_choix.withdraw()  # Masque le menu principal

    def retour_menu():
        fenetre.destroy()
        fenetre_choix.deiconify()  # Affiche à nouveau le menu

    global champ_ip, champ_debut, champ_fin, zone_resultats

    fenetre = tk.Tk()
    fenetre.title("Scanner de Ports")
    ...
    if type_scan == "autre":
        ...
        champ_ip.pack_forget()  # Caché, valeur fixée
    ...
    tk.Button(fenetre, text="Lancer le scan", command=lancer_scan, bg="green", fg="white").pack(pady=10)
